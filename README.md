...
> 🔰 **専門外の方へ**: まず [`docs/00_overview_for_beginners.md`](docs/00_overview_for_beginners.md)
> を読むと、何の役に立つのか（ワルファリンや COVID 治療薬などの実例）と、
> `python scripts/demo_precision_medicine.py` の具体的な出力が分かります。

## ブランチ構成（スタック PR・順序関係）

本リポジトリは **3 段のスタック PR** で開発しており、各ブランチは下の段を土台に積み増します
（依存順 `main ← #1 ← #2 ← #3`）。**最新・全部入りは PR #3（このブランチ）**。下から順
（#1→#2→#3）にマージすると、各 PR のベースが自動で `main` 側へ繰り上がります。
...
📍 **このREADMEは ③ 先端 `claude/advanced-extensions`（PR #3）のものです**（①②③ すべて収録の最新版）。
...
| パス | 内容 |
|---|---|
| `docs/00_overview_for_beginners.md` | 🔰 専門外向け：何の役に立つか（実在薬の例＋デモ出力） |
...
| `docs/11_advanced_extensions.md` | 実用化に向けた4拡張（不確実性/実データ検証/用量・併用/解釈性） |
| `docs/12_running_demos.md` | 実行ガイド（各デモのコマンド・実行時間・出力例・読み方） |
| `src/biomodel/uncertainty.py` | 不確実性の定量化（deep ensemble）＋較正 |
| `src/biomodel/validation.py` | 実データ検証の指標（top-1/regret/responder AUROC） |
| `src/biomodel/dose.py` | 用量反応（Hill）・組み合わせ予測のミニ simulator＋モデル |
| `src/biomodel/interpret.py` | 解釈性（genotype 寄与度）・バイオマーカー検証 |
...
| `scripts/demo_precision_medicine.py` | 🔰 患者ごとの薬の効きを予測（薬の推薦・responder 見分け） |
| `scripts/make_figures.py` | docs 用の結果図を生成（`docs/figures/precision_medicine.png`） |
| `scripts/demo_uncertainty.py` | 不確実性（信頼度）＋較正のデモ |
| `scripts/demo_validation.py` | 実データ検証指標（top-1/regret/AUROC）のデモ |
| `scripts/demo_dose_response.py` | 用量反応・組み合わせ予測のデモ |
| `scripts/demo_interpret.py` | 解釈性・バイオマーカー発見のデモ |
...
| `tests/` | 形状・学習・個人差・損失・前処理・GReX・敵対・anndata・不確実性・検証・用量・解釈性（47 件） |

## クイックスタート

各コマンドは **独立した単体デモ**（1 本の連続実行ではなく、順序依存なし。1 つずつ実行）。
共通の前提は `pip install` だけ。**コマンド・実行時間・出力例・読み方は
[`docs/12_running_demos.md`](docs/12_running_demos.md)** にまとめています。

```bash
python -m pip install -r requirements.txt        # 前提（numpy ＋ できれば torch）

# 基本
python scripts/demo_precision_medicine.py         # 🔰 まずこれ：患者ごとの薬の効きを予測
python scripts/run_demo.py                        # 中核：事前学習→摂動学習→leave-one-donor 評価
python scripts/run_demo.py --observed-rate 0.5    # 疎な観測（未観測ペアは学習から除外）

# 実用化に向けた4拡張（docs/11, docs/12）。--quick で軽量・短時間に実行
python scripts/demo_uncertainty.py --quick        # ① 不確実性＋較正（予測の信頼度）
python scripts/demo_validation.py --quick         # ② 実データ検証指標（top-1/regret/AUROC）
python scripts/demo_dose_response.py --quick      # ③ 用量反応・組み合わせ
python scripts/demo_interpret.py --quick          # ④ 解釈性・バイオマーカー
...
## 実用化に向けた 4 拡張（`docs/11`）

「個人差を予測できる」から「現場で使える」へ近づける拡張。各機能はデモ＋テスト付き。

- **(1) 不確実性の定量化**（`demo_uncertainty.py`）: deep ensemble で予測の信頼度を出し、
  較正で被覆率を nominal に合わせる（生は過信気味の 90%区間→73% を、較正後→91% に補正）。臨床判断の安全性。
- **(2) 実データ検証指標**（`demo_validation.py`）: ex vivo 薬剤感受性（BeatAML 等）を想定し
  top-1 的中・regret・responder AUROC で評価（提案 top1≈0.90, AUROC≈0.95 vs 対照 0.40/0.50）。「本当に当たるか」。
- **(3) 用量・組み合わせ**（`demo_dose_response.py`）: Hill 用量反応で必要量が genotype 依存
  （患者間で約30倍差、予測 r≈1.00）。併用の相乗/拮抗も予測。ワルファリン用量・がん併用に直結。
- **(4) 解釈性・バイオマーカー**（`demo_interpret.py`）: 効果を駆動する genotype 特徴を勾配で抽出し、
  真の駆動因子を ρ≈0.68 で復元。コンパニオン診断の候補マーカーに。
