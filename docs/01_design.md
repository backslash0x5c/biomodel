...
```mermaid
flowchart LR
    scRNA["投与前 scRNA"] -->|"段1: 事前学習 encoder<br/>(MGM)"| zcell["z_cell / z_baseline"]
    geno["genotype + 共変量"] --> zindiv
    zcell --> zindiv["段3: 個人 encoder<br/>z_indiv"]
    drug["薬剤 / 遺伝子摂動"] -->|"段2: 摂動 encoder"| zpert["z_pert"]
    zpert --> phi{{"相互作用 Φ<br/>(FiLM/cross-attn/hypernet)<br/>= 個人差の本体"}}
    zindiv --> phi
    phi --> dec["Decoder"]
    zcell --> dec
    dec --> out["x_pred / Δ<br/>(個別化した処置効果)"]
```
...
$$z_{\text{indiv}} = g\big(\underbrace{e_{\text{geno}}}_{\text{genotype}},\ \underbrace{z_{\text{baseline}}}_{\text{baseline}},\ \underbrace{e_{\text{cov}}}_{\text{covariates}}\big)$$

（genotype = 遺伝型、baseline = 投与前のベースライン状態、covariates = 年齢・性別などの共変量）
...
$$\mathcal{L} = \underbrace{\|\hat{\Delta} - \Delta\|^2}_{\text{recon}} + \lambda_{\text{dist}}\, \mathrm{MMD}(\hat{P}^{\text{pert}}, P^{\text{pert}}) + \lambda_{\text{adv}}\, \mathcal{L}_{\text{batch-adv}}$$
