...
- さらに **GReX（genetically regulated expression）** = PrediXcan/FUSION 流に
  ...
  > 実装済み: `data_pipeline.GReXFeaturizer`（PrediXcan 流の重み $w_{js}$ で GReX を計算）。
  > 重みは `load_predixcan_weights` で `(gene, variant, weight)` 表から読む。PCA フォール
  > バック（`GenotypeFeaturizer("pca")`）より eQTL に忠実。OneK1K への適用は docs/10。
